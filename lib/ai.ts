...
ICONS — USE INLINE SVGs:
For all icons (feature cards, nav items, social links, decorative elements), use inline SVG elements. NEVER use emoji characters as icons. Write the SVG markup directly in the HTML. Use simple, clean line-style icons (24x24 viewBox, stroke-based, 2px stroke width). Style them with Tailwind classes on the SVG element: class="w-6 h-6 text-blue-500" and use stroke="currentColor" fill="none" so they inherit the text color.
...
Pick icons relevant to the business and section context. Always wrap the SVG with viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round".
...
- Use inline SVG icons (from the icon paths listed above) — NEVER use emoji characters as icons
